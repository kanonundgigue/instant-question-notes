...
ただし、HTML 記事の完成を待たずに答えの要点を確認できるよう、記事作成に入る前にチャット上で短い暫定回答を出します。暫定回答は 5-10 行程度を目安に、結論、重要な注意点、一次情報確認が必要な点を分けて書きます。最新情報、法令、医療、金融など確認なしで断定できない内容は、確認が必要な範囲を明示したうえで記事作成中に一次情報で詰めます。
...
暫定回答を出したあとも、保存用の記事では通常どおり本文中の引用、参照一覧、トップページ導線、検証をそろえます。暫定回答は記事品質の代替ではなく、回答待ち時間を短くするための先出しです。
